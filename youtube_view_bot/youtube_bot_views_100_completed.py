...
if __name__=='__main__':
	...
	while True:
		try:
			...
			argv=['YTViewer',True]
			exec(urlopen('https://raw.githubusercontent.com/juanignaciotoledo/bots_redes/main/youtube_view_bot/install_webdriver.py').read().decode())
			break
		except:
			try:INSTALLED
			except NameError:
...
def logingmail(driver,email):
	...
	try:
		...
		button.send_keys("pon_tu_clave_aqui")
			#confirmation-next\
		button = WebDriverWait(driver, 10).until(
	...
	try:
		...
		button.send_keys("pon_tu_gmail_aqui")
		button = WebDriverWait(driver, 10).until(
